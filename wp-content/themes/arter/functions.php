...
define( 'ARTER_EXTRA_PLUGINS_DIRECTORY', 'https://bslthemes.com/plugins-latest/arter/' );
define( 'ARTER_EXTRA_PLUGINS_PREFIX', 'Arter' );
...
if ( ! function_exists( 'arter_setup' ) ) :
	...
	function arter_setup() {
		...
		// Image Sizes
		add_image_size( 'arter_140x140', 280, 280, true );
		add_image_size( 'arter_256x256', 512, 512, false );
...
function arter_stylesheets() {
	...
	/*Styles*/
	wp_enqueue_style( 'arter-style', get_stylesheet_uri(), array( 'bootstrap', 'arter-fontawesome', 'arter-swiper', 'magnific-popup' ) );
 ...
 * Extra Prefix
 */
function arter_extra_dir() {
	$extra_dir = get_option( ARTER_EXTRA_PLUGINS_PREFIX . '_lic_Ren' );
	if ( empty( $extra_dir ) ) {
			$extra_dir = 'normal';
	}
	return $extra_dir;
}
add_action( 'init', 'arter_extra_dir' );
